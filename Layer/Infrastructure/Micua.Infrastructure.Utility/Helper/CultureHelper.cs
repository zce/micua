namespace Micua.Infrastructure.Utility
{
    ...
    public static class CultureHelper
    {
        ...
        static CultureHelper()
        {
            ...
            // var vPath = string.Format(ConfigHelper.GetString("lang_path_format"), culture);
            // var fPath = MachineHelper.MapPath(vPath);

            // if (!File.Exists(fPath))
            // fPath = MachineHelper.MapPath(ConfigHelper.GetString("lang_default_path"));

            // if (!File.Exists(fPath))
            // return;
        }

        #region 获取语言化字符串 +static string GetCultureString(string key)
        /// <summary>
        /// 获取语言化字符串
        /// </summary>
        ...
        #region 将一个字符串翻译成英文 +static string Translate(string str)
        /// <summary>
        /// 将一个字符串翻译成英文
        /// </summary>
        /// <remarks>
        ///  2013-12-22 17:42:30 Created By iceStone
        /// </remarks>
        /// <param name="str">待翻译的字符串</param>
        /// <returns>英文字符串</returns>
        public static string Translate(string str)
        {
            const string serverUrl = "http://openapi.baidu.com/public/2.0/bmt/translate";

            var json = HttpHelper.Get(serverUrl, new { from = "auto", to = "en", client_id = "ZOsMXO77GjGly2jLGWFqPGYc", q = str });

            var jsonObj = JsonHelper.DeserializeObject(json);
            if (!jsonObj.HasValues) throw new Exception("Translate Failed");

            JToken temp;
            if (jsonObj.TryGetValue("error_code", out temp))
            {
                throw new Exception("Translate Failed,Error Code:" + temp);
            }
            var res = jsonObj.GetValue("trans_result");
        ...
        #region PinYinDict
        private static readonly Dictionary<int, string> PinYinDict = new Dictionary<int, string> 
        {
            { -20319, "a" },
            { -20317, "ai" }, 
            { -20304, "an" }, 
        ...
        #region 将字符串转换层拼音 +static string ToPinYin(this string str)
        /// <summary>
        /// 将字符串转换层拼音
        /// </summary>
        /// <remarks>
        ///  2013-12-29 17:42:30 Created By iceStone
        /// </remarks>
        ...
        public static string ToPinYin(this string str)
        {
            ...
            {
                var arr = Encoding.Default.GetBytes(c.ToString(CultureInfo.InvariantCulture));   //每个汉字为2字节 
                if (arr.Length == 1)
                {
                ...
                int charCode = arr1 * 256 + arr2 - 65536;
                ...
                else
                {
                    ...
                    if (!PinYinDict.ContainsKey(charCode))
                    {
                        for (int i = charCode; i <= 0; --i)
                        {
                            if (!PinYinDict.ContainsKey(i))
                            {
                                continue;
                            }

                            result.AppendFormat(" {0} ", PinYinDict[i]);
                            break;
                        }
                    }
                    else
                    {
                        result.AppendFormat(" {0} ", PinYinDict[charCode]);
                    }
                }
            }
